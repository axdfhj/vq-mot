...
class CLIPTextEmbedding(BaseEmbedding):
    ...
    def encode_text(self, text):
        index = clip.tokenize(text, truncate=True).to(self.device)
        index[index < 0] = 0 # some padded text token maybe negative, so set them to 0
        x = self.token_embedding(index).type(self.dtype)  # [batch_size, n_ctx, d_model]
