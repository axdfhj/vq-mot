...
from VQ_utils.transformer_utils import Text2ImageTransformer
eps = 1e-8
...
def log_categorical(log_x_start, log_prob, mask):
    return (log_x_start.exp() * log_prob* mask).sum(dim=1)
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        transformer_config['nb_code'] = nb_code
        self.transformer = Text2ImageTransformer(**transformer_config)
        self.content_seq_len = transformer_config['content_seq_len']
        self.amp = False

        self.num_classes = nb_code + 2 # +2 mask/pad
        self.loss_type = 'vb_stochastic'
        self.shape = transformer_config['content_seq_len']
        ...
        if alpha_init_type == "alpha1":
            at, bt, ct, att, btt, ctt = alpha_schedule(self.num_timesteps, N=self.num_classes-2)
        else:
    ...
    def multinomial_kl(self, log_prob1, log_prob2, mask):   # compute KL loss on log_prob
        kl = ((log_prob1.exp() * (log_prob1 - log_prob2)) * mask).sum(dim=1)
        return kl
    
    def generate_mask(self, length, B, L):
        cum = torch.ones((B, 1, 50)).cumsum(dim=-1).cuda()
        length = length.contiguous().view((B,1,1))
        mask = (cum <= length)
        return mask
    ...
    def q_pred_with_pad(self, log_x_start, t, length):
        bs, _, _ = log_x_start.shape
        # log_x_start can be onehot or not
        t = (t + (self.num_timesteps + 1))%(self.num_timesteps + 1)
        log_cumprod_at = extract(self.log_cumprod_at, t, log_x_start.shape)         # at~
        log_cumprod_bt = extract(self.log_cumprod_bt, t, log_x_start.shape)         # bt~
        log_cumprod_ct = extract(self.log_cumprod_ct, t, log_x_start.shape)         # ct~
        log_1_min_cumprod_ct = extract(self.log_1_min_cumprod_ct, t, log_x_start.shape)       # 1-ct~
        log_probs = torch.zeros_like(log_x_start)
        for i in range(bs):
            temp_log_probs = torch.cat(
                [
                    log_add_exp(log_x_start[i,:-2,:length[i]]+log_cumprod_at[i, :, :], log_cumprod_bt[i, :, :]), # axdfhj
                    log_add_exp(log_x_start[i,-2:-1,:length[i]]+log_1_min_cumprod_ct[i, :, :], log_cumprod_ct[i, :, :]),
                    log_x_start[i, -1:, :length[i]]
                ],
                dim=0
            )
            temp_log_probs = torch.cat([temp_log_probs, log_x_start[i, :, length[i]:]], dim=-1)
            log_probs[i, :, :] = temp_log_probs
        return log_probs

    def predict_start(self, log_x_t, cond_emb, t, length):          # p(x0|xt) changed
        x_t = log_onehot_to_index(log_x_t)
        if self.amp == True:
            with autocast():
                out = self.transformer(x_t, cond_emb, t, length)
        else:
            out = self.transformer(x_t, cond_emb, t, length)
    ...
    def q_posterior(self, log_x_start, log_x_t, t):            # p_theta(xt_1|xt) = sum(q(xt-1|xt,x0')*p(x0'))
        ...
        content_seq_len = log_x_start.size()[-1]
        onehot_x_t = log_onehot_to_index(log_x_t)
        mask = (onehot_x_t == self.num_classes-2).unsqueeze(1)  # self.num_classes-2 == [mask] token
    ...
    def p_pred(self, log_x, cond_emb, t ,length):             # if x0, first p(x0|xt), than sum(q(xt-1|xt,x0)*p(x0|xt))
        if self.parametrization == 'x0':
            log_x_recon = self.predict_start(log_x, cond_emb, t, length)
    ...
    @torch.no_grad()
    def p_sample(self, log_x, cond_emb, t, length):               # sample q(xt-1) for next step from  xt, actually is p(xt-1|xt)
        model_log_prob = self.p_pred(log_x, cond_emb, t, length)
        out = self.log_sample_categorical(model_log_prob, self.num_classes - 1)
        return out

    def log_sample_categorical(self, logits, num_classes):           # use gumbel to sample onehot vector from log probability
    ...
    def q_sample(self, log_x_start, t, num_classes):                 # diffusion step, q(xt|x0) and sample xt // num_classes = self.num_classes - 1
    ...
    def q_sample_with_pad(self, log_x_start, t, length, num_classes):# num_classes = self.num_classes
        log_EV_qxt_x0 = self.q_pred_with_pad(log_x_start, t, length)
    ...
    def compute_probs_losses(self,
        ...
        for index in range(t.size()[0]):
            this_t = t[index].item()
            same_rate = ((x0_recon[index] == x0_real[index]) * maskpad.squeeze(1)).sum().cpu()/x0_real.size()[1]
            self.diffusion_acc_list[this_t] = same_rate.item()*0.1 + self.diffusion_acc_list[this_t]*0.9
            same_rate = ((xt_1_recon[index] == xt_recon[index]) * maskpad.squeeze(1)).sum().cpu()/xt_recon.size()[1]
        ...
        kl = self.multinomial_kl(log_true_prob, log_model_prob, maskpad)
        mask_region = (xt == self.num_classes-2).float()  # self.num_classes - 2 == [mask] token
        mask_weight = mask_region * self.mask_weight[0] + (1. - mask_region) * self.mask_weight[1]
        kl = kl * mask_weight
        kl = sum_except_batch(kl)

        decoder_nll = -log_categorical(log_x_start, log_model_prob, maskpad)
        decoder_nll = sum_except_batch(decoder_nll)

        mask = (t == torch.zeros_like(t)).float().cuda()
        ...
        if self.auxiliary_loss_weight != 0:
            kl_aux = self.multinomial_kl(log_x_start[:,:-1,:], log_x0_recon[:,:-1,:], maskpad)
    ...
    def log_pad(self, w):
        pad = torch.zeros((1, self.num_classes, w)) - 70
        pad[:, -1:, :] = 0
        return pad 
    ...
    def _train_loss(self, x, cond_emb, is_train=True, length=None, loss_weight=None):                       # get the KL loss
        ...
        x_start = x
        t, pt = self.sample_time(b, 'importance')

        log_x_start = index_to_log_onehot(x_start, self.num_classes - 1)
        # log_xt = self.q_sample_with_pad(log_x_start=log_x_start, t=t, length=length, num_classes=self.num_classes)
        log_xt = self.q_sample(log_x_start=log_x_start, t=t, num_classes=self.num_classes-1)
        xt = log_onehot_to_index(log_xt)
        ############### go to p_theta function ###############
        log_x0_recon = self.predict_start(log_xt, cond_emb, t=t, length=length)            # P_theta(x0|xt)

        log_model_prob, vb_loss = self.compute_probs_losses(x_start=x_start,
        ...
        vb_loss /= nb_tokens
        # if decoder_nll > 0:
        #     wandb.log({"decoder_nll" :decoder_nll})
        return log_model_prob, vb_loss


    # @property
    # def device(self):
    #     return self.transformer.module.to_logits[-1].weight.device
    ...
    def parameters(self, recurse=True, name=None):
        ...
        if name is None or name == 'none':
            ...
            param_dict = {pn: p for pn, p in self.transformer.module.named_parameters()}# if p.requires_grad} 
    ...
    # def padding(self, log_z, length):
    #     pad_bottom_logits = torch.zeros((1, 1, length))
    #     pad_bottom_logits = torch.log(pad_bottom_logits)
    #     pad_right_logits = torch.zeros((1, self.num_classes, self.content_seq_len - length))
    #     pad_right_logits[:, -1:, :] = 1
    #     pad_right_logits = torch.log(pad_right_logits)
    #     mask_logits = torch.cat((log_z, pad_bottom_logits), dim=1)
    #     mask_logits = torch.cat((mask_logits, pad_right_logits), dim=-1)
    #     return mask_logits

    def sample_motion_index(
        self,
        cond_emb,
        length,
    ):
    ...
    def sample(
        ...
        if start_step == 0:
            ...
            zero_logits = torch.zeros((batch_size, self.num_classes-1, self.shape))
            one_logits = torch.ones((batch_size, 1, self.shape))
            mask_logits = torch.cat((zero_logits, one_logits), dim=1)
            log_z = torch.log(mask_logits)
            start_step = self.num_timesteps
            with torch.no_grad():
                for diffusion_index in range(start_step-1, -1, -1):
                    t = torch.full((batch_size,), diffusion_index, dtype=torch.long)
                    log_z = self.p_sample(log_z, cond_emb, t)     # log_z is log_onehot

        else:
            t = torch.full((batch_size,), start_step-1, dtype=torch.long)
