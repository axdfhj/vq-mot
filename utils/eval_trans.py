...
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False) : 
        ...
        for batch in tqdm(val_loader):
            ...
            for k in range(bs):
                try:
                    ...
                    index_motion, logits = trans.sample_motion_index(feat_clip_text[k:k+1], torch.tensor([m_token]))
                except:
    ...
    msg = f"--> \t Eva. Iter {nb_iter} :, FID. {fid:.4f}, Diversity Real. {diversity_real:.4f}, Diversity. {diversity:.4f}, R_precision_real. {R_precision_real}, R_precision. {R_precision}, matching_score_real. {matching_score_real}, matching_score_pred. {matching_score_pred}"
    logger.info(msg)
    wandb.log({"fid": fid, "matching_score": matching_score_pred, "r1": R_precision[0], "r2": R_precision[1], "r3": R_precision[2]})
